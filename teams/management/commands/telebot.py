    ...
    try:
        ...
        if not student.in_team:
            ...
            bot.send_message(
                ...
                f"Время созвона: {student.timeslot.first()}.",
            )
    ...
    if not student.in_team:
        ...
        if call.data == "yes":
            ...
            bot.edit_message_text(
                ...
                text="Отлично!\n"
                f"Ты записан к {user_team.pm}: {user_team.pm.tg_username} на {user_team.timeslot.timeslot}.\n"
                "Студенты в твоей команде:\n"
                f"{', '.join([f'{student.name}: {student.tg_username}' for student in user_team.students.all()])}",
                reply_markup=None,
            )
